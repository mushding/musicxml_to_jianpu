...
def readCompressedMusicXML(filename):
    ...
    try:
        container_xml = archive.read('META-INF/container.xml')      # use container.xml to find the data.xml filename
...
class MusicXMLReader:
    ...
    def __init__(self, filename):
        if zipfile.is_zipfile(filename):        # see the xml file is ziped
